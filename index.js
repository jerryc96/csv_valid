...
function parsefile(data, count, cause, validRec, invalidRec){
	...
	if (count == 1){
		...
		if (validcolumns(data)){
			...
			console.log("The header row does not meet PHIX file specifications");
		}
	}
...
function getPath(dir){
	...
	// some check that this is csv file
	var csvStream = csv
		.parse({objectMode:true})
    	.on("data", function(data){
    		count += 1;
    		parsefile(data, count, cause, validRec, invalidRec);
         	//console.log(data);
    	})
    	.on("end", function(result){
			...
			invalidRec.splice(0, 0, csvHead);
         	count = 0;
         	var writeValid = fs.createWriteStream("valid.csv");
         	csv.write(validRec, {headers:true}).pipe(writeValid);
         	var writeinValid = fs.createWriteStream("invalid.csv");
         	csv.write(invalidRec, {headers:true}).pipe(writeinValid);
         	var writeCause = fs.createWriteStream("problems.txt");
         	for (n in cause){
